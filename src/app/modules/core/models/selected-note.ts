...
export class SelectedNote {
  ...
  constructor(root: DirectoryRoot, folder: DirectoryFolder | undefined, note: DirectoryNote) {
    this._root = root;
    this._folder = folder;
    this._note = note;
  }
  ...
  public get noteFilePath(): string {
    let path = `${this._root.path}${this._folder?.path ?? ''}${this._note.name}`;
    if (!path.endsWith(DirectoryNote.extension)) {
      path += DirectoryNote.extension;
    }
    return path;
  }
}
